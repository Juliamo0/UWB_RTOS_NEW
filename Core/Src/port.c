...
void spi_peripheral_init()
{
    ...
    /* LCD not connected - no initialization needed */
}
...
{
    // Not implemented - no BOOT1 pin in this configuration
    return 0;
}
...
{
    // Not implemented - no switch in this configuration
    (void)GPIOpin;
    return 0;
}
...
{
    // No LEDs connected in this configuration
    (void)led;
}
 ...
 *          Note: DWM1000 module does not expose NSS pin
 * */
void port_wakeup_dw1000(void)
{
    // DWM1000 module does not expose NSS pin
    // Alternative wakeup method may be needed if device enters sleep mode
    Sleep(8); // Wait for XTAL to stabilize
}
 ...
 * @brief   set slow SPI speed for initialization
 *          note: hspi1 is clocked from 84MHz (STM32F446)
 * */
void port_set_dw1000_slowrate(void)
{
    hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32; // ~2.6 MHz
    HAL_SPI_Init(&hspi1);
}

/* @fn      port_set_dw1000_fastrate
 * @brief   set fast SPI speed for normal operation
 *          note: hspi1 is clocked from 84MHz (STM32F446)
 * */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    // Add DW_IRQn_Pin handler here if IRQ pin is connected
    // Example:
...
{
    // If IRQ pin is configured, read its state here
    // Example: return HAL_GPIO_ReadPin(GPIOx, GPIO_PIN_X);
    return 0;
}
...
/****************************************************************************//**
 *
 *                              USB report section (NOT USED)
 *
 *******************************************************************************/

/* USB CDC functionality removed - not used in this configuration */

/* @fn      port_tx_msg()
 * @brief   Stub function - USB not used
 * */
 ...
 *
 * NOTE:
 *   - As the IRQ handler does not check that port_deca_isr is not null, the user application must ensure that a
